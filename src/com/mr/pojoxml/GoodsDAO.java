...
public class GoodsDAO extends BaseHibernateDAO {
	...
	public Long save(Goods transientInstance) {
		log.debug("saving Goods instance");
		try {
			getSession().save(transientInstance);
			log.debug("save successful");
			return transientInstance.getGid();
	...
	public Goods findById(java.lang.Long id) {
		...
		try {
			Goods instance = (Goods) getSession().get("com.mr.pojoxml.Goods",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Goods instance) {
		...
		try {
			List results = getSession().createCriteria("com.mr.pojoxml.Goods")
	...
	public Page<Goods> findGoodsByGoodssearch(GoodsSearch goodsSearch,Page<Goods> page){
		try{
			StringBuffer hql = new	StringBuffer("from Goods g where 1=1");
			...
			if(goodsSearch.getGtype()!=null){
				hql.append(" and gtype.gtypeid = "+goodsSearch.getGtype().getGtypeid());
			}
			if(goodsSearch.getImgCount()!=null){
				hql.append(" and g.imgses.size >= :imgCount");
			}
	...
	public Long allResult(GoodsSearch goodsSearch){
		StringBuffer hql = new	StringBuffer("select count (*) from Goods g where 1=1");
		if(goodsSearch.getGname()!=null){
			hql.append(" and gname like :gname");
		}
		if(goodsSearch.getMax()!=null){
		...
		if(goodsSearch.getGtype()!=null){
			hql.append(" and gtype ="+goodsSearch.getGtype().getGtypeid());
		}
		if(goodsSearch.getImgCount()!=null){
			hql.append(" and g.imgses.size >= :imgCount");
		}
